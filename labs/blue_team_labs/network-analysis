...
[text](../../../../labs/blueteamlabs/DdMgCqiLCvTxMd6XYQgaXCyjDH8M6b/BTLOPortScan.pcap)

- Notes:

a local host shoudnt be scanning another host, unless it is an authorized host that is used for vulnerbility scans
...
- 04. Two more tools were used to perform reconnaissance against open ports, what were they?
    ...
    - I filtered by a command such "ip.src == 10.251.96.4 && http.request.method == POST", then I look at 13979 package:
        User-Agent: sqlmap/1.4.7#stable (http://sqlmap.org) --> a tool use to setup automatically some sql commands to collect some data with username=user&password=pass.
...
- 04. What is the name of the php file through which the attacker uploaded a web shell? 
    - editprofile.php
...
- 06. What is the parameter used in the web shell for executing commands?
    - in the 16134th package: 
        GET /uploads/dbfunctions.php?cmd=id HTTP/1.1\r\n --> paramater is "cmd"

- 07. What is the first command executed by the attacker?
    - in the 16134th package: 
        GET /uploads/dbfunctions.php?cmd=id HTTP/1.1\r\n --> first command is "id"

- 08. What is the type of shell connection the attacker obtains through command execution?
    - reverse shell: is a technique where a compromised host opens an outbound connection to an attacker-controlled system and then provides shell access back over     that connection.

- 09. What is the port he uses for the shell connection? 
